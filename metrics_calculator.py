...
class MetricCalculator:
    ...
    def get_mape(self):
        self.mape = np.mean(np.abs((self.label - self.predict) / self.label)) * 100

    # 4 准确率
    def get_accuracy(self):
        count = 0
        for (yt, yp) in zip(self.label, self.predict):
            if np.abs(yt - yp) < 0.25:
                count += 1
        return count / len(self.predict)
    ...
    def print_metrics(self):
        ...
        if self.mape: print("mape", self.mape)
        if self.get_accuracy(): print("accuracy", self.get_accuracy())
